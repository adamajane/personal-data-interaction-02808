...
def write_night_sessions(records, out_dir):
    ...
    monthly_night = defaultdict(
        lambda: {"hours": 0.0, "tracks": 0, "artists": defaultdict(float)}
    )
    for r in records:
        ym = r["year_month"]
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for ym in sorted(monthly_total.keys()):
            ...
            top_artist = (
                max(nd["artists"], key=nd["artists"].get) if nd["artists"] else ""
            )
            w.writerow(
...
if __name__ == "__main__":
    ...
    print(
        f"  → {len(parsed):,} valid records  ({skipped:,} skipped — under {MIN_MS//1000}s)"
    )
    ...
    print(
        f"  → Night streams (22h–04h): {len(night):,} plays, "
